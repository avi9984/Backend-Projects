...
const authorization = async (req, res, next) => {
  try {
    ...
    if (req.body.hasOwnProperty("userId")) {
      ...
      if (!userData)
        return res.status(404).send({status: false,message: "Error! Please check user id and try again",
          });
    ...
    if (req.params.hasOwnProperty("bookId")) {
      ...
      if (!bookData)
        return res.status(404).send({status: false,message: "Error! Please check book id and try again",
          });
