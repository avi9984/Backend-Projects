...
// const {sdk}=require("./awsController")
...
const createBook = async function (req, res) {
  try {
    ...
    if (isValidBody(data)) {
      return res
        .status(400)
        .send({ status: false, message: "Enter Book details" });
    }

    if (data.hasOwnProperty("reviews")) {
      return res
        .status(400)
        .send({ status: false, message: "Cannot edit review" });
    }
    ...
    if (!data.releasedAt) {
      return res
        .status(400)
        .send({ status: false, message: "ReleasedAt date is required" });
    }
    ...
    if (
      validString(data.excerpt) ||
      validString(data.category) ||
      validString(data.subcategory)
    ) {
      return res.status(400).send({
        status: false,
        message: "Data should be valid and does not contains numbers",
      });
    }
    ...
    if (validDate(data.releasedAt)) {
      return res.status(400).send({
        status: false,
        message: "Enter a valid released date in (YYYY-MM-DD) formate",
      });
    }
...
const getFilteredBooks = async (req, res) => {
  try {
    ...
    let getFilterBooks = await Book.find(data).sort({ title: 1 }).select({
      title: 1,
      excerpt: 1,
      userId: 1,
      category: 1,
...
const updateBook = async (req, res) => {
  try {
    ...
    if (isValidBody(data))
      return res.status(400).send({
        status: false,
        message: "Data is required to update document",
      });
    ...
    if (
      ...
      return res
        .status(400)
        .send({ status: false, message: "Action is Forbidden" });
    ...
    if (
      ...
      return res
        .status(400)
        .send({ status: false, message: "Data should not contain Numbers" });
    }
    ...
    let updatedBookData = await Book.findByIdAndUpdate({ _id: bookId }, data, {
      new: true,
    });
...
const deleteBook = async function (req, res) {
  try {
    ...
    if (book.isDeleted == true) {
      return res.status(404).send({
        status: false,
        message: "Book not found or has already been deleted",
      });
    }
