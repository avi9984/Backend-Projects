    ...
    if (data.hasOwnProperty("address")) {
      if (isValidObjectType(data.address))
        return res.status(400).send({
          status: false,
          message:
            "Enter address in a object with 'street','city' and 'pincode' as keys",
        });
    }
...
const userLogin = async function (req, res) {
  try {
    ...
    if (!checkValidUser) {
      return res
        .status(400)
        .send({ status: false, message: "Email Id is not correct " });
    }
    ...
    let token = jwt.sign({ userId: checkValidUser._id }, "Books-Management", {
      expiresIn: "1d",
    });
