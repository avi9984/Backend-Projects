  ...
  try {
    ...
    if (validString(data.reviewedBy) || validString(data.review)) {
      return res
        ...
        .send({
          status: false,
          message: "Enter valid data in review and reviewedBy",
        });
    }

    //Checking that rating should be only in numbers and between 1 to 5
    if (!validString(data.rating))
      return res
        .status(400)
        .send({ status: false, message: "Rating should be in numbers" });
    if (!(data.rating < 6 && data.rating > 0))
      return res
        .status(400)
        .send({
          status: false,
          message: "Rating should be between 1 - 5 numbers",
        });
    ...
    await Book.updateOne({ _id: bookId }, { $inc: { reviews: 1 } });
...
const updateReview = async (req, res) => {
  try {
    ...
    if (!isValidObjectId(getID.reviewId))
      return res
        .status(400)
        .send({ status: false, message: "Enter a valid Review id" });
    ...
    if (!checkReviewId)
      return res
        ...
        .send({
          status: false,
          message: "Review not found check id and try again",
        });
    ...
    if (checkReviewId.bookId.toString() !== getID.bookId)
      return res
        ...
        .send({
          status: false,
          message: "Book not found check id and try again",
        });
    ...
    if (checkReviewId.isDeleted == true)
      return res
        ...
        .send({
          status: false,
          message: "Review not found or might have been deleted",
        });
    ...
    if (
      data.hasOwnProperty("bookId") ||
      data.hasOwnProperty("isDeleted") ||
      data.hasOwnProperty("reviewedAt")
    )
      return res
        .status(400)
...
const deleteReview = async (req, res) => {
  try {
    ...
    //Updating the review
    await Review.updateOne({ _id: getID.reviewId }, { isDeleted: true });

    //Updating the reviews count in Book by decrease it by 1
    await Book.updateOne({ _id: getID.bookId }, { $inc: { reviews: -1 } });
